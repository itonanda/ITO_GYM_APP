...
export default function MemberDashboardScreen() {
  ...
  useEffect(() => {
    async function loadStoredSession() {
      try {
        ...
        if (error) {
          ...
          // Alert.alert('Session', storedSession);
          Alert.alert('Role', storedSession.user?.user_metadata?.role || 'member');
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, currentSession) => {
       Alert.alert('Session', event);
      if (event === 'SIGNED_IN' || event === 'SIGNED_OUT' || event === 'USER_UPDATED') {
        console.log("Session:", currentSession);
        console.log("mengambil sesi simpanan:", currentSession?.user?.user_metadata?.role || 'member');
        // Alert.alert('Session', currentSession);
        // Alert.alert('Role', currentSession.user?.user_metadata?.role || 'member');
  ...
  useEffect(() => {
    async function checkCurrentSession() {
      try {
        // Ambil data user dari sesi aktif yang tersimpan di storage lokal
        const { data: { user: currentUser }, error } = await supabase.auth.getUser();

        if (error) {
          // Jika token hilang/invalid, system akan menangkapnya di sini
          console.log("Sesi tidak ditemukan atau kedaluwarsa:", error.message);
          return;
        }

        if (currentUser) {
          setUser(currentUser);
          console.log("Sesi aktif ditemukan untuk email:", currentUser.email);
        }
      } catch (err) {
  ...
  useEffect(() => {
    async function loadDashboardData() {
      try {
        // 1. Ambil data user yang sedang login dari local storage (AsyncStorage)
        const { data: { user }, error: userError } = await supabase.auth.getUser();
        
        if (userError) throw userError;
      //   if (userError) {
      //   // Jika session kosong atau hilang, jangan tampilkan Alert error ke user
      ...
      // }
        ...
        // 2. CONTOH: Mengambil data tambahan dari tabel database menggunakan ID user
        if (user) {
          const { data, error: dbError } = await supabase
            .from('profiles') // Ganti dengan nama tabel database Anda
            .select('*')
            .eq('id', user.id)
            .single();
