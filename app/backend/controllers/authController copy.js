...
export const signUp = async (req, res) => {
  try {
    ...
    // if (error) return res.status(400).json({ error: error.message });
    res.status(200).json({ success: true, message: 'Check your email for the confirmation link!', Email });
  } catch (error) {
...
export const signIn = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, message: 'Sigin successfully', User });
...
export const signOut = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, message: 'Signed out successfully', User });
    // res.cookie("access_token", data.session.access_token, { httpOnly: true });
    // const { email } = req.params;
    // const user = await getUserByEmail(email);
    // if (!user) {
...
export const changePassword = async (req, res) => {
  try {
    ...
    const changePassword = await changePasswordService(userData);
